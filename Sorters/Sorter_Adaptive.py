...
class Sorter_PingPong_Adaptive(Sorter_PingPong.Sorter_PingPong):
    def __init__(self, input_list, k,
        ...
        self.runs_ping = self._detect_runs()
    ...
    def sort(self):
        ...
        num_read_runs = len(self._get_runs_read_list())
        num_write_runs = len(self._get_runs_write_list())
        ...
        while num_write_runs > 1:
            ...
            while posn_in_read_runs < num_read_runs:
                ...
                this_block_runs = self._get_runs_read_list()[block_number * self.k:block_end]
                ...
                if len(this_block_runs) > 1:
                    ...
                    this_block_write_list_slice = ListSlice.ListSlice(self._get_write_list(), write_start, write_end)
                    ...
                    self._get_runs_write_list()[posn_in_write_runs] = this_block_write_list_slice
                    posn_in_write_runs += 1

                    block_number += 1
                else:
                    # one run is inherently sorted, so just copy it over
                    self._get_runs_write_list()[posn_in_write_runs] = this_block_runs[0]
                    posn_in_write_runs += 1
    ...
    def _str_runs_read_list(self, length):
        runs_read_list = self._get_runs_read_list()
        output = []
    ...
    def _str_runs_write_list(self, length):
        runs_write_list = self._get_runs_write_list()
        output = []
        for run_number in range(length):
