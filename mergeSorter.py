# todo:
# 1. find the next element for k-way merging via an optimal method.
...
class mergeSorter:
	...
	def mergeK(self, runs):
		...
		while len(runs) > 1:
			...
			self.sendToOutput(workingOutList,currentList[runs[minRun][0]])
		...
		while runs[0][0] != runs[0][1]:
			self.sendToOutput(workingOutList,currentList[runs[0][0]])
			runs[0][0] += 1
