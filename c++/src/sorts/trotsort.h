...
namespace algorithms {
	...
	private:
		...
		TrotSort(size_t len) : minGallop_(MIN_GALLOP) {
			/*
		...
		void mergeAt(diff_t const i) {
			...
			merge_runs(base1, base2, base2 + len2, buffer_.begin());
	 ...
	 */
	template<typename Iterator,bool useBinaryInsertionsort = false>
	class trotsort final : public sorter<Iterator> {
	public:
		void sort(Iterator begin, Iterator end) override {
			typedef typename std::iterator_traits<Iterator>::value_type value_type;
			TrotSort<Iterator,useBinaryInsertionsort>::sort(begin, end);
		}
